...
A = int(input('Enter whole number coefficient "A" in the equation"AX^3 + BX^2 + CX + D" :'))
B = int(input('Enter whole number coefficient "B" in the equation"AX^3 + BX^2 + CX + D" :'))
C = int(input('Enter whole number coefficient "C" in the equation"AX^3 + BX^2 + CX + D" :'))
D = int(input('Enter whole number coefficient "D" in the equation"AX^3 + BX^2 + CX + D" :'))
...
if A==0:
    ...
    bound_a = float(input("Enter bound 'a' (negative):"))
    bound_b = float(input("Enter bound 'b' (positive):"))
...
#when one of the bounds that user input is root
    if (fa > 0 and fb < 0) or (fa < 0 and fb > 0):
        while abs(length) >= minimum:
            p = ((bound_a+bound_b)/2.0)
            fp = (A * (p ** 3)) + (B * (p ** 2)) + (C * p) + D

            if fa > 0 and fp < 0:
                #redefine P and get a new polynomial
                bound_b = p
                fb = (A * (bound_b ** 3)) + (B * (bound_b ** 2)) + (C * bound_b) + D
            elif fb > 0 and fp < 0:
                #redefine P and get a new polynomial
            ...
            length = fa-fb #redefine the length because one of the bounds changed 
            
        print("The root is at x =",'{:g}'.format(float('{:.1g}'.format(p))),"and it took",counter,"bisections to find it.")
    
    elif abs(fa-rootc) <= minimum or abs(fb-rootc) <= minimum:
